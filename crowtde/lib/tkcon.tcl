...
proc ::tkcon::Init {args} {
	...
	showOnStartup	1
	slaveprocs	{
	    alias clear dir dump echo idebug lremove observe observe_var unalias which what tkcon_puts tkcon_gets
	}
...
proc ::tkcon::InitUI {title} {
    ...
    #edit by dai
    #catch {font create tkconfixed -size -12}
    #catch {font create tkconfixedbold -size -12 -weight bold}
...
proc ::tkcon::MainInit {} {
	...
	#edit by dai-----s
	$tmp eval [list set ::tkcon::PRIV(appPath) $::tkcon::PRIV(appPath)]
	$tmp eval {
	    if [catch {source -rsrc tkcon}] { 
		    source [file join $::tkcon::PRIV(appPath) "tools" "tkcon.tcl"]
	    	    ::tkcon::Init
	    }
	}
	#---------------e
...
proc ::tkcon::Bindings {} {
	...
	#edit by dai
	#%W mark set insert [%W index insert]
...
#	puts root=[info exists PRIV(root)]
#	puts arg0=[info exists ::argv0]
#	puts arg0=$::argv0
	puts script=$PRIV(SCRIPT)
    if {(![info exists PRIV(root)] || ![winfo exists $PRIV(root)]) \
...
proc ::tkcon::AtCrow {path scriptPath} {
	variable PRIV
	set PRIV(root) $path
	set PRIV(SCRIPT) $scriptPath
	set PRIV(appPath) [file dirname [file dirname $scriptPath]]
	set sfont [::crowFont::get_font smaller]
