...
proc twapi::unlock_user {username args} {
    # UF_LOCKOUT -> 0x10
    eval [list _change_usri3_flags $username 0x10 0] $args
}
...
proc twapi::enable_user {username args} {
    ...
    eval [list _change_usri3_flags $username 0x2 0] $args
}
...
proc twapi::disable_user {username args} {
    # UF_ACCOUNTDISABLE -> 0x2
    eval [list _change_usri3_flags $username 0x2 0x2] $args
}
...
proc twapi::get_user_local_groups_recursive {account args} {
    ...
    return [NetUserGetLocalGroups $opts(system) [map_account_to_name $account -system $opts(system)] 1]
}
