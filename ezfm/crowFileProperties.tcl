...
proc ::libCrowFM::show {path fpath} {
	variable vars
	variable wInfo
	...
	foreach bit "suid sgid sticky" {
		checkbutton $fme.chk$bit -text $bits($bit) -anchor w \
			-variable ::libCrowFM::vars(sid,$bit)
	...
	foreach acl "owner group other" {
		...
		foreach bit "r w x" {
			checkbutton $fme.chk$bit -text $attrs($bit) -anchor w \
				-variable ::libCrowFM::vars($acl,$bit)
			if {($mask & $finfo(mode))==0} {
				set vars($acl,$bit) 0
			} else {
				set vars($acl,$bit) 1
	...
	set vars(user) [::libCrowFM::uid_to_name $finfo(uid)]
	set vars(group) [::libCrowFM::gid_to_name $finfo(gid)]
	set vars(recursive) 0
	ComboBox $fme.cmbUser -textvariable ::libCrowFM::vars(user) \
		-values [::libCrowFM::get_users] -editable false 
	ComboBox $fme.cmbGroup -textvariable ::libCrowFM::vars(group) \
		-values [::libCrowFM::get_groups] -editable false 
	checkbutton $fme.chkRecursive -text [::msgcat::mc "recursive apply"] -anchor w \
		-variable ::libCrowFM::vars(recursive) -state disabled
	...
	button $fmeButton.btnOk -text [::msgcat::mc "Accept"] -command [list ::libCrowFM::btn_accept_click $fpath]
...
proc ::libCrowFM::btn_accept_click {path} {
	variable vars
	variable wInfo
	::libCrowFM::chown $path $vars(user) $vars(group) $vars(recursive)
	set mode 0
	...
	::libCrowFM::chmod $path $mode $vars(recursive)
	$wInfo(dialog) enddialog ""
}

proc ::libCrowFM::chown {path user group recursive} {
	array set finfo ""
	file stat $path finfo
	set uname [::libCrowFM::uid_to_name $finfo(uid)]
	set gname [::libCrowFM::uid_to_name $finfo(gid)]
...
proc ::libCrowFM::chmod {path mode recursive} {
	array set finfo ""
	file stat $path finfo
