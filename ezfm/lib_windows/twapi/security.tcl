...
proc twapi::get_ace_text {ace args} {
    ...
    if {[lsearch -glob $rights *_all_access]} {
        set rights "All"
    } else {
...
proc twapi::get_resource_security_descriptor {restype name args} {
    ...
    set type [_map_resource_symbol_to_type $restype]
    ...
    set secd [Twapi_GetNamedSecurityInfo $name $type $wanted]
...
proc twapi::set_resource_security_descriptor {restype name secd args} {
    ...
    SetNamedSecurityInfo $name $type $mask $opts(owner) $opts(group) $opts(dacl) $opts(sacl)
}
...
proc twapi::_map_resource_symbol_to_type {sym} {
    ...
    switch -exact -- $sym {
        ...
        registry  { set type 4 }
        share     { set type 5 }
        kernelobj { set type 6 }
        default { error "Unknown resource type '$restype'" }
    }
    return $type
}
