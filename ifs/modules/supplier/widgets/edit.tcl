oo::class create ::dApp::supplier::edit {
	...
	method find {} {
		...
		set filter "name LIKE '%$keyword%' OR  id LIKE '%$keyword%' OR tel1 LIKE '%$keyword%' LIMIT 300"
	...
	method save {} {
		...
		if {$Row(id) == ""} {
			my Msg_Put [::msgcat::mc "無法修改不存在的資訊。"]
		}

		set id $Row(id)
		unset Row(id)
		set ret [$db set $id {*}[array get Row]]
		set Row(id) $id
	...
	method Ui_Body_fmeL_Init {} {
		...
		set lblId [my Label_Create $fmeBasic.lblId -text [::msgcat::mc "供應商代號"] -foreground "#808080"]
		...
		set lblCreateDate [my Label_Create $fmeBasic.lblCreateDate -text [::msgcat::mc "建檔時間"] -foreground "#808080"]
		...
		set txtBusinessItem [text $fmeBusinessItem.txtNote -relief groove -bd 2 -height 5 -width 1 -wrap none]
...
#		grid $lblBusinessItem $fmeBusinessItem -sticky new -padx 2 -pady 3
		...
		set txtNote [text $fmeNote.txtNote -relief groove -bd 2 -height 5 -width 1 -wrap none]
...
#		grid $fmeNote -row 14 -column 3 -rowspan 2 -sticky new -padx 2 -pady 3
	...
	method Ui_Body_fmeR_Init {} {
		...
		set lblTitle [ttk::label $fme.lblTitle \
			-text [::msgcat::mc "搜尋結果"] \
			-justify center \
			-anchor center \
		]
		...
		set btnApply [ttk::button $fmeTv.btnApply \
			...
			-compound left \
			-style Toolbutton \
			-command [list [self object] edit $tvFind] \
		]
		...
		bind $tvFind <<ButtonL-DClick>> [list [self object] edit $tvFind] 
		...
		set btnApply [ttk::button $fme.btnApply \
			...
			-compound left \
			-style Toolbutton \
			-command [list [self object] edit $tvRecent] \
		]
		...
		bind $tvRecent <<ButtonL-DClick>> [list [self object] edit $tvRecent]
...
#			-text [::msgcat::mc "重新填寫 (F2)"] \
#			-command [list [self object] reset] \
#			-style Toolbutton]	
		
		pack $btnSave  -side left -padx 6 -pady 2
		...
		bind $txtFind <FocusIn> [list [self object] Help_Put [::msgcat::mc "請輸入供應商編號、供應商名稱或供應商電話包含的內容。"]]
